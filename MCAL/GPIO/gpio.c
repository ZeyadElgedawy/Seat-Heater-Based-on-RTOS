...
void GPIO_ButtonInit(void)
{
    ...
    GPIO_PORTA_CR_REG    |=  (1<<4);
    GPIO_PORTA_PCTL_REG  &= 0xFFF0FFFF;
    GPIO_PORTA_DIR_REG   &= ~(1<<4);
    GPIO_PORTA_AFSEL_REG &= ~(1<<4);
    GPIO_PORTA_PUR_REG   |=  (1<<4);
    GPIO_PORTA_DEN_REG   |=   0x10;
    GPIO_PORTA_AMSEL_REG &=   0x10;
}
...
uint8 GPIO_ExtButtonGetState(void)
{
    return ((GPIO_PORTA_DATA_REG >> 4) & 0x01);
}
...
void GPIO_ExternalEdgeTriggeredInterruptInit(void)
{
    GPIO_PORTA_IS_REG    &= ~(1<<4);      /* PA4 detect edges */
    GPIO_PORTA_IBE_REG   &= ~(1<<4);      /* PA4 will detect a certain edge */
    GPIO_PORTA_IEV_REG   &= ~(1<<4);      /* PA4 will detect a falling edge */
    GPIO_PORTA_ICR_REG   |= (1<<4);       /* Clear Trigger flag for PA4 (Interrupt Flag) */
    GPIO_PORTA_IM_REG    |= (1<<4);       /* Enable Interrupt on PA4 pin */
